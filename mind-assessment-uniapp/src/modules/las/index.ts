...
function computeResult(answers: any[], qs: any[]) {
  ...
  let maxDim = 'eros';
  let maxScore = -1;
  Object.entries(dimScores).forEach(([k, v]: [string, any]) => {
    if (v > maxScore) {
      maxScore = v;
      maxDim = k;
    }
  ...
  const dominant = DIMENSIONS[maxDim as keyof typeof DIMENSIONS];
...
export default {
  ...
  buildInterpretations(r: any) {
    const top3 = Object.entries(r.groups)
      ...
      .slice(0, 3)
      .map(([k]: [string, any]) => r.groupDetails[k]);

    return [
      { title: '主导风格', text: `你的主导爱情风格是${r.dominantName}型（${r.dominant.icon}）。${r.description}` },
      { title: '风格排序', text: `你最突出的三种风格依次是：${top3.map((d: any) => `${d.name}(${d.sum}分)`).join('、')}。` },
  ...
  getDimensionLabel(dim: string) {
    const map: any = { eros: '激情', ludus: '游戏', storge: '友谊', pragma: '实用', mania: '狂热', agape: '奉献' };
    return map[dim] || dim;
  },
