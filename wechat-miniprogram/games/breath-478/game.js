...
Component({
  ...
  methods: {
    ...
    nextPhase() {
      ...
      let color = '#14b8a6'
      if (p.label === '吸气') {
        initialScale = 0.8 // start small when inhalation
        color = '#3b82f6'
      } else if (p.label === '呼气') {
        initialScale = 1.2 // start big for exhalation
        color = '#ef4444'
      }
      // For hold (屏息), maintain the current circleScale from previous phase
    ...
    runPhaseTimer(sec) {
      ...
      this._progress = setInterval(() => {
        ...
        if (phase === '吸气') {
          ...
          // Stay still - circleScale remains whatever it was
