 ...
 *        抑郁：正常<10 / 轻度10-13 / 中度14-20 / 重度21-27 / 极重度≥28
 *        焦虑：正常<8 / 轻度8-9  / 中度10-14  / 重度15-19 / 极重度≥20
 *        压力：正常<15 / 轻度15-18 / 中度19-25 / 重度26-33 / 极重度≥34
 *
...
const DIM_INFO = {
  // P0-3: 修正为标准 DASS-42 等效分阈值
  depression: { name: '抑郁', en: 'Depression', thresholds: [10, 14, 21, 28] },
  anxiety:    { name: '焦虑', en: 'Anxiety',    thresholds: [8, 10, 15, 20] },
  stress:     { name: '压力', en: 'Stress',     thresholds: [15, 19, 26, 34] },
};

// 严重度排序（用于取最突出的维度）
const LEVEL_ORDER = ['正常', '轻度', '中度', '重度', '极重度'];

function describeLevel(dimension, dass42) {
...
function computeResult(answers, qs) {
  let answered = 0;
  const items = [];
  const dimRaw = { depression: 0, anxiety: 0, stress: 0 };
...
module.exports = {
  ...
  buildInterpretations(r, groupList, scaleDimensionList) {
    ...
    const arr = [
      ...
      { name: '焦虑', ...a },
      { name: '压力', ...s },
    ].sort((x, y) => LEVEL_ORDER.indexOf(y.level) - LEVEL_ORDER.indexOf(x.level));
    const worst = arr[0];
    ...
    return [
      ...
      { title: '分级说明', text: '各维度采用 DASS-42 等效分阈值：抑郁 10/14/21/28、焦虑 8/10/15/20、压力 15/19/26/34，依次划分为正常、轻度、中度、重度、极重度。' },
