...
- **云同步占位真实化与死代码清理（P0/P1/P3）**：`utils/user.js` 增加 `CLOUD_ENABLED` 开关并导出 `isCloudEnabled()`，所有云端同步调用在未接入时静默跳过，不再向用户展示「同步成功」等不实状态；移除 `mine`/`profile` 中未绑定且无实效的「同步」处理函数（其 WXML 本就无对应入口，故不影响现有界面）；首页/关于页明确「数据仅本机、未启用云同步」；删除 `theme-store.js` 从未被调用的 `applyTheme`；清理 `tools/map-questions.js`/`analyze.js` 未使用变量，使 `npm run lint` 达成 **0 error / 0 warning**。
...
- **舒尔特方格点击偶发不识别（修复）**：`games/schulte/game.wxml` 棋盘加 `catchtouchmove`，避免手指轻微滑动被页面当成滚动、导致 `bindtap` 被取消（网格类游戏在可滚动页上最常见的「吞点」根因，最大 9×9 棋盘整体可显示、无需在盘内滚动，故安全）；`onTap` 中将 `dataset.n/idx` 显式转 `Number`，规避数据集偶发字符串类型导致 `n === next` 比较失败。同根因的视觉搜索/数字划消/找方格/图形记忆等网格点选游戏如需可一并加 `catchtouchmove`。
