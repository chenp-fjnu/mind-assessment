...
function computeResult(answers: any[], qs: any[]) {
  ...
  return {
    dimensions: dims,
    raw: Object.values(dims).reduce((a: number, d: any) => a + d.sum, 0),
    total: qs.length,
    totalStat: { correct: 0, wrong: 0, skipped: answers.filter((a: any) => a == null).length },
    type: Object.keys(dims).map((d: string) => (dims as any)[d].level === 'high' ? d : ((dims as any)[d].level === 'low' ? d.toLowerCase() : d + '=')).join(''),
    typeName: '五维人格画像',
    trait: `O${(dims as any).O.percent}% C${(dims as any).C.percent}% E${(dims as any).E.percent}% A${(dims as any).A.percent}% N${(dims as any).N.percent}%`,
    description: `开放性${(dims as any).O.percent}%、尽责性${(dims as any).C.percent}%、外向性${(dims as any).E.percent}%、宜人性${(dims as any).A.percent}%、神经质${(dims as any).N.percent}%。`,
    groups: {
      O: (dims as any).O.sum, C: (dims as any).C.sum, E: (dims as any).E.sum, A: (dims as any).A.sum, N: (dims as any).N.sum,
    },
  };
}
...
export default {
  ...
  getDimensionLabel(dim: string) {
    const labels: Record<string, string> = { O: '开放性', C: '尽责性', E: '外向性', A: '宜人性', N: '神经质' };
