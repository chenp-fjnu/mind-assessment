...
function computeResult(answers: any[], qs: any[], timings: number[] = []) {
  // 按子测验统计
  const subScores: Record<string, { correct: number; total: number; domain: string }> = {};
  ...
  return {
    ...
    subScores,
    groups: {
      言语: Object.entries(subScores).filter(([, s]) => s.domain === 'verbal').reduce((a: number, [, s]) => a + s.correct, 0),
      操作: Object.entries(subScores).filter(([, s]) => s.domain === 'performance').reduce((a: number, [, s]) => a + s.correct, 0),
    },
    totalStat,
...
export default {
  ...
  buildSubtestList(r: any) {
    if (!r.subScores) return [];
    return Object.entries(r.subScores).map(([name, sub]: [string, any]) => ({
      name,
      domain: sub.domain,
  ...
  // 维度标签：韦氏使用子测验名作为标签
  getDimensionLabel(dim: string) { return dim; },
