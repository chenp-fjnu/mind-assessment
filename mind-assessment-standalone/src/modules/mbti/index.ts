...
const DIM_INFO: Record<string, { left: string; right: string; leftName: string; rightName: string; leftDesc: string; rightDesc: string }> = {
...
function computeResult(answers: any[], qs: any[]) {
  ...
  return {
    ...
    dimensions: dims,
    counts,
    raw: Object.values(counts).reduce((a: number, b: number) => a + b, 0),
    total: qs.length,
    totalStat: { correct: 0, wrong: 0, skipped: qs.length - Object.values(counts).reduce((a: number, b: number) => a + b, 0) },
  };
}

function opposite(pole: string) {
  return ({ E: 'I', I: 'E', S: 'N', N: 'S', T: 'F', F: 'T', J: 'P', P: 'J' } as Record<string, string>)[pole];
}
...
export default {
  ...
  getDimensionLabel(dim: string) {
    const labels: Record<string, string> = { EI: '外向/内向', SN: '实感/直觉', TF: '思考/情感', JP: '判断/感知' };
